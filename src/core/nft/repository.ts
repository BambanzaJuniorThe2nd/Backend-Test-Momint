...
export class NFTs implements NFTRepository {
  ...
  async getAllByUserIds(userIds: string[], limit: number): Promise<NFT[]> {
    try {
      const result = await this.collection
        .find({
          userId: { $in: userIds.map((userId) => new ObjectId(userId)) },
        })
        .limit(limit);
      return await result.toArray();
    } catch (e) {
  ...
  async getOwnerById(id: string): Promise<NFTOwner> {
    ...
    try {
      ...
      if (!nft.contract || !nft.tokenId) {
        throw new CoreError(
          messages.ERROR_NFT_WITHOUT_CONTRACT,
      ...
      return await contract.ownerOf(nft.tokenId._hex);
    } catch (e) {
      if (e instanceof CoreError) {
        throw e;
      }
